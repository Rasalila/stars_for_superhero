...
superheroes.forEach((obj, index) => {//проходит по каждому объекту массива и отображает его на странице
    const newDiv = document.createElement('div');
    newDiv.classList.add('hero');
    newDiv.innerHTML = `
        <h2>${obj.name}</h2>
        <p><b>Вселенная:</b> ${obj.universe}</p>
        <p><b>Альтер-эго:</b> ${obj.alterego}</p>
        <p><b>Род деятельности:</b> ${obj.occupation}</p>
        <p><b>Друзья:</b> ${obj.friends}</p>
        <p><b>Суперсилы:</b> ${obj.superpowers}</p>
        <div><img src="${superheroes[index].url}" alt='${obj.name}'></div>
        <div><img src=${obj.url} alt='${obj.name}'></div>
        <div class="ratings">
...
const ratings = localStorage.getItem('ratings') || [];//бёрем массив из локальной памяти или создаём новый
...
for (let star of stars) {
    star.addEventListener('click', function(){
        ...
        this.setAttribute('data-clicked', true);//когда нажимаем на звезду, у неё появляется атрибу-ориентир
        
        let rating = this.dataset.rating;//переменная rating получает значение атрибута data-rating у текущего элемента(от 1 до 5)
        let hero = this.closest('.hero');//Находим ближайший элемент с классом 'hero'
        let name = hero.querySelector('h2').textContent;//Извлекаем имя супергероя из элемента h2
